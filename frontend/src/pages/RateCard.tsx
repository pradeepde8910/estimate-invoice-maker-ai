  ...
  return (
    <div className="flex-1 bg-transparent min-h-screen">
          ...
          {!draft ? (
            ...
            <ul className="divide-y divide-slate-100">
              {Object.entries(draft).map(([key, r]) => (
                <li key={key} className="flex flex-wrap items-center justify-between gap-y-2 py-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        inputMode="numeric"
                        value={r.rate_per_hour.toString()}
                        onChange={(e) => {
                          const val = e.target.value.replace(/\D/g, '').replace(/^0+(?=\d)/, '')
                          updateRate(key, Math.min(9999999, Number(val || '0')))
                        }}
                    ...
                    <button onClick={() => setDeleteTarget(key)} className="text-slate-300 hover:text-coral-500 text-lg w-6 flex justify-center pb-1" title="Remove role">×</button>
                  </div>
                </li>
              ))}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <input
                      type="text"
                      inputMode="numeric"
                      value={newRoleRate ? newRoleRate.toString() : ''}
                      onChange={(e) => {
                        const val = e.target.value.replace(/\D/g, '').replace(/^0+(?=\d)/, '')
                        setNewRoleRate(Math.min(9999999, Number(val || '0')))
                      }}
                      placeholder="8000"
      ...
      <ConfirmModal
        ...
        title="Remove role?"
        message={`Are you sure you want to remove the "${deleteTarget && draft?.[deleteTarget]?.label}" role from the rate card?`}
        confirmText="Remove"
        onConfirm={() => deleteTarget && handleRemoveRole(deleteTarget)}
