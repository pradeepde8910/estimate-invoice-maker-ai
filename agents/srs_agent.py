...
async def _call_groq(text: str, system_prompt: str, max_retries: int = 3) -> str:
    ...
    raise Exception(f"All Groq API attempts failed for SRS Agent. Last error: {last_error}")
...
async def srs_node(state: PipelineState) -> dict:
    ...
    try:
        srs_text = await _call_groq(context, SRS_SYSTEM_PROMPT)
        log.append("   ✅ SRS generation complete (via Groq)")
        return {
        ...
        log.append(f"   ⚠️ Groq SRS generation failed ({e}), attempting Gemini fallback...")
        try:
            srs_text = await _call_gemini_fallback(context, SRS_SYSTEM_PROMPT)
            log.append("   ✅ SRS generation complete (via Gemini Fallback)")
            return {
                "srs_markdown": srs_text,
                "log": log,
                "current_stage": "srs_complete"
            ...
            log.append(f"   ❌ SRS generation failed: {gemini_err}")
            return {
                "errors": [f"SRS generation error: {str(gemini_err)}"],
                "log": log,
