...
async def brd_node(state: PipelineState) -> dict:
    ...
    try:
        brd_text = await _call_gemini(context, BRD_SYSTEM_PROMPT)
        log.append(f"   ✅ BRD successfully generated ({len(brd_text)} chars)")
        return {
        ...
        log.append(f"   ❌ BRD generation failed: {e}")
        return {
            "errors": [f"BRD generation error: {str(e)}"],
            "log": log,
