...
from db import init_db, SessionLocal, User, Client, Estimation, Document, Invoice, RateCard, generate_next_serial, AuditLog
...
@app.patch("/api/estimations/{id}/client")
async def patch_estimation_client(id: str, payload: EstimationClientPatch, request: Request):
    db = SessionLocal()
    try:
        require_role(request, db, {"Admin", "Developer"})
...
async def list_db_clients():
    ...
    try:
        ...
        unique_clients = []
        for c in clients:
            if not c.company_name: continue
