...
def calculate_invoice_financials(
    subtotal: Decimal, total_gst: Decimal, tds_amount: Decimal, discount_amount: Decimal = Decimal('0.00')
) -> dict:
    """
    Computes the final invoice totals.
    (Subtotal - Discount) + GST = Gross Amount
    Gross Amount - TDS = Net Receivable (Total Payable)

    `subtotal` is returned unchanged (the full billed amount against the
    milestone/component) — it's what validate_billing_ceiling and
    component.billed_amount track against the contract value, so it must
    not be reduced by the discount.
    """
    ...
    gross_amount = subtotal - discount_amount + total_gst
